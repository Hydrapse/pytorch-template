...
train_loader = ShaDowKHopSampler(data, depth=2, num_neighbors=20,
                                 node_idx=data.train_mask, **kwargs)
val_loader = ShaDowKHopSampler(data, depth=2, num_neighbors=20,
                               node_idx=data.val_mask, **kwargs)
test_loader = ShaDowKHopSampler(data, depth=2, num_neighbors=20,
...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, root_n_id):
        edge_index = dropout_edge(edge_index, p=0.05, training=self.training)
...
model = GNN(num_features, 256, num_classes).to(device)
